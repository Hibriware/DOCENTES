...
import {FECHA_ACTUAL} from '../../servicios/api'
...
const Entregas = () => {
    ...
    return (
        <div>
            <Chip size="small" icon={<ErrorIcon/>} label="El periodo activo será asignado al sistema de alumnos"
                  color="secondary"/>
            <h3 style={{textAlign: 'center'}}>CALENDARIO DE ENTREGA DE CALIFICACIONES</h3>
            <h4 style={{textAlign: 'center'}}>PERIODO
                ACTIVO:{PERIODO ? PERIODO.rango + PERIODO.anio : 'NO DISPONIBLE'}</h4>
                ...
                <Card elevation={3}>
                    ...
                    <Container>
                        <Grid container spacing={5}>
                            ...
                            <Grid item xs={4}>
                                <Paper className={classes.paper} elevation={0}>
                                    <Editar disabled={!status} isDisable={setStatus} periodos={periodos}
                                            setIsStatus={setIsStatus} handleDisables={handleDisables}/>
