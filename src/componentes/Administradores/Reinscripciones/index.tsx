...
import BuscarAlumno from './BuscarAlumno/index';
import {
    AvailableSubject,
    CourseType,
    ReenrollmentResult,
    ...
    useEffect(() => {
        if (student?.folio) {
            ...
            axios.get(`${COURSED_SUBJECTS_URL}/${student?.folio}`).then(res => {
                ...
                //console.log('COURSED_SUBJECTS_URL',res.data)
            }).catch(() => {
    ...
    const fetchAvailableSubjects = useCallback(() => {
        ...
        if (student?.folio && student.career.id && periodos) {
            axios.get(AVAILABLE_SUBJECTS_URL, {
                ...
                //console.log('AVAILABLE_SUBJECTS_URL',value.data)
            })
    ...
    const fetchAvailableSubjectsAcademic = useCallback(() => {
        ...
        if (student?.folio && periodos) {
            axios.get(ACADEMIC_CHARGE_ADMIN_URL, {
                ...
                //console.log('ACADEMIC_CHARGE_ADMIN_URL',value.data)
...
//TODO:listas disponibles para mostrar en la carga academica subjects
    useEffect(() => {
...
//TODO: listas filtrada sin las materias aprobadas
    ...
    useEffect(() => {
        // console.log("materias disponibles",filteredSubjects)
        /* let mylist =   filteredSubjects.map(item=>
             selectedChargeByStudent.some(selectedByStudent => {
                 if (selectedByStudent.materiadocente_id === item.subject.materiaDocente_id){
                     return true;
                 }else{
                     console.log(selectedByStudent)
                     return false
                 }
    ...
    const handleSelection = async (data: AvailableSubject[]) => {
        await setLoaders(true)
        ...
        let cruces = 0;
        await data.forEach((availableSubject, index) => {
        ...
        await setSelectedSubjects(data);
        await setLoaders(false)
    }
                    ...
                    <Grid container spacing={2} className={classes.mt}>
                        <Grid container direction="row" justify="center" alignItems="center">
                            <Grid>
                                {
                                    hoursCruse ?
                                        <Chip color="secondary" size="small" style={{justifyContent: 'center'}}
                                              icon={<WarningIcon/>}
