...
const aprobacion = async (calcularTemas) => {
		...
		switch (cont) {
			case 1:
				var temaTotal = newListaParciales.filter(function(tem) {
					//sumaTema = sumaTema + (tem.tema1 >= 70 ? tem.tema1:0);
					return tem.tema1 >= 70;
				});
				//aprobacionTema1 = Math.round((temaTotal.length / dataReporteParciales.length)*100)
				const proemedioAprobacion = (Math.round(100 / dataReporteParciales.length * temaTotal.length))
				aprobacionTema1 = proemedioAprobacion;
	...
	console.log(aprobacionTotal,"aprobacionTotal");
	console.log(reprobacionTotal,"reprobacionTotal")
